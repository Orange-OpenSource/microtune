...
from pkg.policy import CtxPolicy
from pkg.actions import Actions

# Create class object for a single linear ucb disjoint arm
class dlinucb_disjoint_arm():
    
    def __init__(self, arm_index, d, delta, gamma, _lambda):
        ...
        self.A = self._lambda * np.identity(d)
        self.A_tilde = self._lambda * np.identity(d)
    ...
    def calc_UCB(self, x_array, step):
        numerateur = 1 - (self.gamma**(2*(step-1)))
        denominateur = self._lambda * self._d * (1 - self.gamma**2) 
        beta_t =  np.sqrt(self._lambda) + np.sqrt((2 * log(1 / self.delta)) + self._d * np.log(1 + (numerateur/denominateur)))
        ...
        confid = np.dot(x.T, np.dot(A_inv,np.dot(self.A_tilde,np.dot(A_inv,x))))
        ...
        p = np.dot(self.theta.T,x) + beta_t * np.sqrt(confid)
    ...
    def reward_update(self, reward, x_array):
        ...
        self.A = self.gamma * self.A + np.dot(x, x.T) + ((1 - self.gamma) * self._lambda * np.identity(self._d))
        self.A_tilde = self.gamma**2 * self.A_tilde + np.dot(x, x.T) + ((1 - self.gamma**2) * self._lambda * np.identity(self._d))
...
# Discounted LinUCB
# See paper: Weighted Linear Bandits for Non-Stationary Environments (Yoan Russac)
class DLinUCBPolicy(CtxPolicy):
    
    def __init__(self, actions: Actions | tuple = (-1, 1), ctx=[], delta=0.1, gamma=0.1,_lambda=1,seed=None, use_tips=True):
        super().__init__(actions, True, ctx=ctx, use_tips=use_tips, seed=seed)    # Use tips for LinUCB
        self.linucb_arms = [dlinucb_disjoint_arm(arm_index = i, d = self._nfeatures, delta = delta, gamma = gamma, _lambda = _lambda) for i in range(self._k_arms)]
        self._tstep=1
    ...
    def select_arm(self, x_array, deterministic=False, debug=False):
        ...
        for arm_index in range(self.getArmsCount()):
            ...
            arm_ucb = self.linucb_arms[arm_index].calc_UCB(self._getPolicyFeatures(x_array), self._tstep)
